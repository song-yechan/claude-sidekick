  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      const {
        data: summaryData,
        error: summaryError
      } = await supabase.functions.invoke('summarize-text', {
        body: {
          text: editedContent
        }
      });
          ...
          <CardContent className="p-6 space-y-4">
            {isEditing ? <>
                <div>
                  ...
                  <Input id="page-number" type="number" placeholder="예: 42" value={editedPageNumber} onChange={e => setEditedPageNumber(e.target.value)} className="mt-1" />
                </div>

                <div>
                  <div className="flex items-center justify-between mb-1">
                    <Label htmlFor="summary">AI 요약</Label>
                    <Button variant="outline" size="sm" onClick={handleGenerateSummary} disabled={isGeneratingSummary || !editedContent.trim()} className="gap-1">
                      {isGeneratingSummary ? <>
                  ...
                  <Textarea id="summary" placeholder="AI 요약 생성 버튼을 눌러 요약을 생성하세요" value={editedSummary} onChange={e => setEditedSummary(e.target.value)} className="min-h-[80px] mt-1" />
                </div>

                <div>
                  <Label htmlFor="content">전체 내용</Label>
                  <Textarea id="content" value={editedContent} onChange={e => setEditedContent(e.target.value)} className="min-h-[150px] mt-1" />
                </div>
                ...
                <div>
                  ...
                  <Textarea id="memo" placeholder="이 문장에 대한 생각을 기록해보세요" value={editedMemo} onChange={e => {
                ...
                {note.pageNumber && <div className="pb-3 border-b border-border">
                    <p className="text-sm font-medium text-muted-foreground">
                      페이지 {note.pageNumber}
                    </p>
                  </div>}
                
                {note.summary && <div className="pb-4 border-b border-border">
                ...
                {note.memo && <div className="pt-4 border-t border-border bg-primary-light/30 -mx-6 -mb-6 px-6 py-4 rounded-b-lg">
                    <h3 className="text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">
                      메모장 
                    </h3>
                    <p className="text-base text-foreground whitespace-pre-wrap leading-relaxed">
